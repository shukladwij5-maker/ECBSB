...
async function initGesture() {
    ...
    try {
        ...
        if (camStatus) camStatus.innerText = "Cam Disabled";
...
async function detectHands() {
    ...
    if (predictions.length > 0) {
        ...
        const currentX = hand.landmarks[9][0];
...
function handleResize() {
    ...
    if (isPortrait) {
        gsap.to(pageGroup.rotation, { z: -Math.PI / 2, duration: 0.5 });
        gsap.to(camera.position, { z: 20, duration: 0.5 });
    } else {
        gsap.to(pageGroup.rotation, { z: 0, duration: 0.5 });
        gsap.to(camera.position, { z: 14, duration: 0.5 });
    }
}
